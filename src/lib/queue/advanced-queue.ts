...
import { playwrightScraper, PlaywrightConfig, PlaywrightResult } from '../scraper/playwright-engine';
...
class AdvancedScrapingQueue {
  ...
  private async processJob(job: Job<ScrapingJobData>): Promise<JobResult> {
    ...
    try {
      ...
      const result: PlaywrightResult = await playwrightScraper.scrape(configWithTimeout);
      ...
      const jobResult: JobResult = {
        ...
        metrics: {
          ...
          complianceChecks: result.complianceResult ?
            result.complianceResult.warnings : []
        },
